...
function updateConnectionStatus() {
  ...
  const label = connection.connected
    ? `PM3 verbunden · ${connection.port || "auto"} · ${connection.target || "PM3"}`
    : connection.status === "lost"
      ? "Verbindung verloren"
      : connection.status === "checking"
        ? "Verbindung wird geprüft ..."
        : "Kein Proxmark erkannt";
...
function renderAntennaResult(result) {
  ...
  return `
    <div class="detail-list">
      <div class="detail-row"><span>LF-Antenne</span><span>${escapeHtml(lf.status || "unbekannt")}</span></div>
      ...
      <div class="detail-row"><span>HF-Antenne</span><span>${escapeHtml(hf.status || "unbekannt")}</span></div>
...
async function startAntennaCheck() {
