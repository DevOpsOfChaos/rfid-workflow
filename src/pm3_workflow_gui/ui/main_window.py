...
class MainWindow(QMainWindow):
    ...
    def _build_main_screen(self) -> None:
        ...
        self.normal_mode_button = QPushButton("Normalmodus")
        ...
        self.expert_mode_button = QPushButton("Expertenmodus")
        self.expert_mode_button.setCheckable(True)
        self.expert_mode_button.setObjectName("modeButton")
        self.mode_group = QButtonGroup(self)
        self.mode_group.setExclusive(True)
        self.mode_group.addButton(self.normal_mode_button)
        self.mode_group.addButton(self.expert_mode_button)
        self.normal_mode_button.clicked.connect(lambda: self._set_mode("normal"))
        self.expert_mode_button.clicked.connect(lambda: self._set_mode("expert"))
        ...
        header.addStretch(1)
        header.addWidget(self.normal_mode_button)
        header.addWidget(self.expert_mode_button)
        ...
        self.pages.addWidget(self._expert_technologies_page())
        self.pages.addWidget(self._expert_tools_page())
        self.pages.addWidget(self._expert_templates_page())
        self.pages.addWidget(self._expert_analysis_page())
        self.pages.addWidget(self._expert_log_page())
    ...
    def _write_page(self) -> QWidget:
        ...
        self.write_authorized = QCheckBox("Schreibmodus aktivieren")
        self.write_authorized.toggled.connect(self._render_write_plan)
        self.write_integrity_note = QLabel(
            "Änderungen werden einzeln ausgeführt und danach erneut gelesen. Hardwareseitig nicht schreibbare Bereiche bleiben unverändert."
        )
        self.write_integrity_note.setObjectName("muted")
        self.write_integrity_note.setWordWrap(True)
        ...
        top_panel_layout.addWidget(self.write_integrity_note, 2, 3, 1, 2)
    ...
    def _expert_technologies_page(self) -> QWidget:
        page = QWidget()
        layout = QVBoxLayout(page)
        layout.setContentsMargins(18, 14, 18, 12)
        title = QLabel("Technologien")
        title.setObjectName("pageTitle")
        subtitle = QLabel("Capability-Matrix aus registrierten Adaptern")
        subtitle.setObjectName("muted")
        self.capability_table = QTableWidget(0, 7)
        self.capability_table.setHorizontalHeaderLabels(
            ["Technologie", "Erkennen", "Basisdaten", "Detailread", "Vorlage", "Schreiben", "Analyse"]
        )
        self.capability_table.setEditTriggers(QAbstractItemView.NoEditTriggers)
        _polish_table(self.capability_table)
        layout.addWidget(title)
        layout.addWidget(subtitle)
    ...
    def _expert_tools_page(self) -> QWidget:
        page = QWidget()
        layout = QVBoxLayout(page)
        layout.setContentsMargins(18, 14, 18, 12)
        title = QLabel("Werkzeuge")
        title.setObjectName("pageTitle")
        subtitle = QLabel("Registrierte PM3-Funktionen mit kontrollierten Parametern")
        subtitle.setObjectName("muted")
        selector_row = QHBoxLayout()
        selector_row.addWidget(QLabel("Technologie"))
        self.expert_technology_select = QComboBox()
        for row in capability_matrix_view_model():
            self.expert_technology_select.addItem(row.technology, row.technology_id)
        self.expert_technology_select.currentIndexChanged.connect(lambda _index: self._render_expert_tools())
        selector_row.addWidget(self.expert_technology_select)
        selector_row.addStretch(1)
        self.expert_tools_table = QTableWidget(0, 4)
        self.expert_tools_table.setHorizontalHeaderLabels(["Werkzeug", "Zustand", "Parameter", "Technische Erklärung"])
        self.expert_tools_table.setEditTriggers(QAbstractItemView.NoEditTriggers)
        _polish_table(self.expert_tools_table)
        layout.addWidget(title)
        layout.addWidget(subtitle)
        layout.addLayout(selector_row)
        layout.addWidget(self.expert_tools_table, 1)
        self._render_expert_tools()
        return page

    def _expert_templates_page(self) -> QWidget:
        page = QWidget()
        layout = QVBoxLayout(page)
        layout.setContentsMargins(18, 14, 18, 12)
        title = QLabel("Vorlagen & Dumps")
        title.setObjectName("pageTitle")
        subtitle = QLabel("Vorlagen, Dumps, Backups, Vergleiche und Audit-Protokoll")
        subtitle.setObjectName("muted")
        table = QTableWidget(5, 2)
        table.setHorizontalHeaderLabels(["Bereich", "Regel"])
        table.setEditTriggers(QAbstractItemView.NoEditTriggers)
        rows = [
            ("Vorlagen", "Technologie-neutrale Metadaten plus adapterbezogene Felder"),
            ("Dumps", "Rohdaten getrennt von bestätigten Vorlagen halten"),
            ("Backups", "Vor jeder echten Änderung Vorher-Read sichern, wenn der Adapter lesen kann"),
            ("Vergleiche", "Nur technisch relevante Unterschiede hervorheben"),
            ("Audit-Protokoll", "Keine Zugangsdaten oder Keys in normale Statusmeldungen schreiben"),
        ]
    ...
    def _expert_analysis_page(self) -> QWidget:
        page = QWidget()
        layout = QVBoxLayout(page)
        layout.setContentsMargins(28, 22, 28, 18)
        title = QLabel("Analyse")
        title.setObjectName("pageTitle")
        summary = QLabel("Signal-, Antennen- und Erkennungswerkzeuge")
        summary.setObjectName("messagePanel")
        ...
        for index, (heading, text, enabled, callback) in enumerate(cards):
            grid.addWidget(self._analysis_card(heading, text, enabled, callback), index // 2, index % 2)
        layout.addWidget(title)
        layout.addWidget(summary)
        layout.addLayout(grid)
        return page

    def _expert_log_page(self) -> QWidget:
        page = QWidget()
        layout = QVBoxLayout(page)
        layout.setContentsMargins(18, 14, 18, 12)
        title = QLabel("Protokoll")
        title.setObjectName("pageTitle")
        self.expert_log = QPlainTextEdit()
        self.expert_log.setReadOnly(True)
        self.expert_log.setPlainText("Noch keine technischen Details erfasst.")
        layout.addWidget(title)
        layout.addWidget(self.expert_log, 1)
        return page

    def _render_capability_matrix(self) -> None:
        rows = capability_matrix_view_model()
        self.capability_table.setRowCount(len(rows))
        for row_index, row in enumerate(rows):
            cells = [row.technology, row.detect, row.identity, row.detail_read, row.template, row.write, row.analysis]
            for col, cell in enumerate(cells):
                if col == 0:
                    item = QTableWidgetItem(cell)
                else:
                    item = QTableWidgetItem(cell.state_label)
                    item.setToolTip(cell.explanation)
                    background, foreground = _capability_colors(cell.state)
                    item.setBackground(background)
                    item.setForeground(foreground)
                self.capability_table.setItem(row_index, col, item)
        self.capability_table.resizeColumnsToContents()

    def _render_expert_tools(self) -> None:
        technology_id = self.expert_technology_select.currentData() if hasattr(self, "expert_technology_select") else None
        tools = expert_tools_view_model(technology_id)
        self.expert_tools_table.setRowCount(len(tools))
        for row_index, tool in enumerate(tools):
            values = [tool.label, tool.state_label, tool.parameter_hint, tool.explanation]
            for col, value in enumerate(values):
                item = QTableWidgetItem(value)
                if col == 1:
                    background, foreground = _capability_colors(tool.state)
                    item.setBackground(background)
                    item.setForeground(foreground)
                self.expert_tools_table.setItem(row_index, col, item)
        self.expert_tools_table.resizeColumnsToContents()
    ...
    def _set_mode(self, mode: str) -> None:
        ...
        self.nav.clear()
        if mode == "expert":
            self.nav.addItems([f"▣  {item}" for item in expert_navigation_items()])
            self.expert_mode_button.setChecked(True)
            self.pages.setCurrentIndex(3)
            self._set_status("Bereit · Expertenmodus")
        else:
            self.nav.addItems([f"▣  {item}" for item in normal_navigation_items()])
            self.normal_mode_button.setChecked(True)
            self.pages.setCurrentIndex(0)
            self._set_status("Bereit · Normalmodus")
    ...
    def _render_write_plan(self) -> None:
        ...
        if selected < 0 or selected >= len(self._templates):
            ...
            self._set_status("Bereit · keine Vorlage ausgewählt")
            self.disabled_actions.addStretch(1)
            return
        ...
        if self._target_scan is None:
            ...
            self.action_status.setText(activation.reason)
            self._set_status("Bereit · Zielchip fehlt")
            self.disabled_actions.addStretch(1)
            return
        if self._target_scan.profile is None:
            ...
            self.action_status.setText(activation.reason)
            self._set_status("Bereit · kein Schreibplan für diesen Chip")
            self.disabled_actions.addStretch(1)
            return
        ...
        if activation.write_ready:
            self._set_status("Vergleich erstellt · Schreibmodus aktiv")
        else:
            self._set_status(f"Bereit · {activation.reason}")
    ...
    def _confirm_hitag_write(self, action) -> None:
        ...
        if not self.write_authorized.isChecked():
            QMessageBox.warning(self, "Schreiben", "Schreibmodus ist nicht aktiv.")
            return
    ...
    def _change_page(self, row: int) -> None:
        if row >= 0:
            self.pages.setCurrentIndex(row + (3 if self._mode == "expert" else 0))
    ...
    def _append_raw(self, result) -> None:
        ...
        if hasattr(self, "expert_log"):
            self.expert_log.setPlainText("\n\n".join(self._raw_log) or "Noch keine technischen Details erfasst.")
    ...
    def _show_help(self) -> None:
        QMessageBox.information(
            self,
            "Hilfe",
            "Normalmodus: scannen, Vorlagen erstellen, vergleichen und technisch verfügbare Schreibbereiche verifizieren. Expertenmodus: registrierte PM3-Funktionen strukturiert nach Technologie, ohne freie Terminal-Shell.",
        )
    ...
    def _apply_style(self) -> None:
        self.setStyleSheet(
            ...
            QPushButton#modeButton { padding: 4px 9px; font-weight: 600; }
            QPushButton#modeButton:checked { background: #245d8f; color: #ffffff; }
...
def _capability_colors(state: str) -> tuple[QColor, QColor]:
    colors = {
        "available": ("#e8f5ec", "#14532d"),
        "requires_known_credentials": ("#fff7cc", "#5f4500"),
        "hardware_locked": ("#eef1f4", "#4b5563"),
        "not_implemented_yet": ("#eef2ff", "#3730a3"),
